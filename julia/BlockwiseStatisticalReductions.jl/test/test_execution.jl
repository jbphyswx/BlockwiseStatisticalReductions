Test.@testset "simple execution" begin
    ...
    Test.@test results[1] isa Vector  # Multiple window results
    Test.@test length(results[1]) == 4  # 2x2 grid of windows
end
...
Test.@testset "execute_node for WindowNode" begin
    ...
    Test.@test length(result) == 4  # 2x2 windows
    Test.@test result[1] isa ReductionResult
end

Test.@testset "execute_node for StatsNode" begin
    ...
    Test.@test result isa ReductionResult
    Test.@test result.data ≈ mean(arr)
end

...
```
"""
struct ScaleResults{N,NT<:NamedTuple,W<:AbstractVector,R<:AbstractVector}
    input_shape::NTuple{N,Int}
    windows::W
    results::R
    plan::Plan{N}
end
ScaleResults(input_shape::NTuple{N,Int}, windows, results, plan) where {N} =
    ScaleResults{N,eltype(results),typeof(windows),typeof(results)}(input_shape, windows, results, plan)
...
Base.eltype(::Type{<:ScaleResults{N,NT,W}}) where {N,NT,W} = Pair{eltype(W),NT}
...
Where the windows of one result sit: the `window`, the per-axis `origins` (0-based) and the input index
`ranges` each output cell covers along every axis.
"""
function geometry(r::ScaleResults{N}, key) where {N}
    w = _window_for(r, key)
    return (window = w, origins = map(origins, w), ranges = ntuple(d -> [window_range(w[d], i) for i in 1:nwindows(w[d])], Val(N)))
end
